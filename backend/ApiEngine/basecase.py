...
try:
    ...
    from . import tools as global_func
...
class GenerateCase:
    ...
    def create_test_name(self, index, length):
        ...
        # 使用zfill方法进行补零
        test_name = f"test_{str(index + 1).zfill(padding)}"
        return test_name
...
class BaseTest(unittest.TestCase, CaseRunLog):
    ...
    def __request_log(self):
        ...
        self.info_log('请求响应状态码：{}'.format(self.status_code))
    ...
    def save_env_variable(self, name, value):
        ...
        if DEBUG:
            self.warning_log('提示：调试模式运行，设置的临时变量会保存在测试环境的【调试】变量中')
            ENV[name] = value
        else:
            self.env[name] = value
    ...
    def assertion(self, methods, expected, actual):
        ...
        self.info_log('--------------------------------断言--------------------------------')
        self.debug_log('断言方式：{}'.format(methods))
        ...
        try:
            if assert_method:
                ...
                raise TypeError('断言比较方法为：{}，当前断言方式不支持！'.format(methods))
    ...
    def __run_script(test, data):
        ...
        if teardown_script:
            try:
                ...
                test.error_log('断言脚本执行错误：\n{}'.format(e))
                raise
        yield

    def __run_teardown_script(self, response):
        """执行断言脚本"""
        self.info_log('==================================执行断言脚本==================================')
...
def run_test(case_data, env_config, thread_count=1, debug=True):
    ...
    :return:
        debug模式：会返回本次运行的结果和本次运行设置的全局变量，
    """
