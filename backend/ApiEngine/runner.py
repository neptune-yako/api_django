...
class TestResult(unittest.TestResult):
    ...
    def startTest(self, test):
        ...
        getattr(test, 'info_log')("开始执行测试用例：【{}】\n".format(test.name))
    ...
    def stopTest(self, test):
        ...
        :return:
        """
        test.run_time = '{:.2}s'.format((time.time() - self.start_time))
    ...
    def addSuccess(self, test):
        ...
        test.state = '成功'
        getattr(test, 'info_log')("测试用例：{}，执行结果为：【通过】\n".format(getattr(test, '_testMethodDoc')))
    ...
    def addFailure(self, test, err):
        ...
        self.result["fail"] += 1
        test.state = '失败'
        getattr(test, 'warning_log')("测试用例：{}，执行结果为：【失败】\n".format(getattr(test, '_testMethodDoc')))
    ...
    def addError(self, test, err):
        ...
        getattr(test, 'error_log')("测试用例：{}，执行结果我为：【错误】\n".format(getattr(test, '_testMethodDoc')))
